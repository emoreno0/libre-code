...
export default function App() {
  ...
  return (
    <div className="block p-4 space-y-4 space-x-4">
      <button onClick={openFile} className="p-1 text-lg bg-black text-white rounded-md">
        Open file
      </button>
      <button onClick={openFolder} className="p-1 text-lg bg-black text-white rounded-md">
        Open folder
      </button>
      {content && path != undefined ?
        <button onClick={removeValues} disabled={!content} className="p-1 text-lg bg-black text-white rounded-md">
          Remove
        </button>
        :
        <></>
      ...
      <div className="text-black space-y-4 font-mono text-sm whitespace-pre-wrap">
        <div>
          {content ? (
            content.map((phrase) => <p>{phrase}</p>)
          ) : (
            <p>No content to show</p>
          )}
        </div>
        {path ?
          <p>{path}</p> : <p>No path to show</p>
        }
      </div>
    </div>
  )
